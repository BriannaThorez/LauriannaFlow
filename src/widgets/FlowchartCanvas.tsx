...
const FlowchartCanvasInner = () => {
  ...
  useFrame((state, delta) => {
    ...
    // Apply zoom velocity with friction for "industry leading" feel
    if (Math.abs(zoomVelocity.current) > 0.0001 || Math.abs(cam.zoom - targetZoom.current) > 0.001) {
      const oldZoom = cam.zoom;
      
      // Update target based on velocity
      targetZoom.current *= (1 + zoomVelocity.current);
      targetZoom.current = THREE.MathUtils.clamp(targetZoom.current, 0.5, 150);
      
      // Decay velocity
      zoomVelocity.current *= Math.pow(0.0001, delta); // Smooth friction independent of frame rate
      
      // Lerp actual zoom to target for smoothness
      const lerpFactor = 1 - Math.pow(0.0000001, delta);
      cam.zoom = THREE.MathUtils.lerp(cam.zoom, targetZoom.current, lerpFactor);
      
      // Zoom-to-cursor logic: keep the point under the mouse fixed in world space
      const mouseWorld = new THREE.Vector3(mouse.x, mouse.y, 0).unproject(cam);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (editingId) return;
      if (e.key === '+' || e.key === '=') {
        zoomVelocity.current += 0.0125; // Reduced by 75% (from 0.05)
      }
      if (e.key === '-' || e.key === '_') {
        zoomVelocity.current -= 0.0125; // Reduced by 75% (from 0.05)
      }
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Accelerated response: small scrolls are precise, large scrolls are fast
      const power = e.ctrlKey ? 1.5 : 1.2; // Extra boost if holding ctrl
      const acceleratedDelta = sign * Math.pow(absDelta, power) * 0.000125; // Reduced by 75% (from 0.0005)
      
      zoomVelocity.current += acceleratedDelta;
    };
  ...
  const handlePointerMove = (e: any) => {
    ...
    if (isRotating && selectedId && e.nativeEvent.buttons !== 0) {
      ...
      if (shape) {
        ...
        updateShape(selectedId, { rotation: startRotation + delta });
      }
      return;
    }
