...
import { 
  ...
  Grid3X3, Type, ArrowRightLeft, Package 
} from 'lucide-react';
...
export const Toolbar = () => {
  const { setActiveTool, activeTool, mode, setMode, shapes, links, themeName, setThemeName } = useFlowchartStore();
  const [showThemeMenu, setShowThemeMenu] = useState(false);

  // Dynamically create SVG on each change
  const currentSVG = useMemo(() => generateSVG(shapes, links), [shapes, links]);
  ...
  const tools: { id: FlowchartState['activeTool'], icon: any, label: string, description: string, shortcut: string }[] = [
    ...
    { id: 'vertex', icon: Grid3X3, label: 'Vertex Edit', description: 'Manually adjust the geometry of selected shapes.', shortcut: 'V' },
    ...
    <div className="absolute top-4 left-4 flex flex-col gap-4 items-start z-[100]">
      ...
      <div className="bg-background/60 backdrop-blur-xl p-1.5 rounded-xl border border-primary/10 flex gap-1.5 shadow-2xl items-center">
        <SmartTooltip content="Studio Mode" description="Full creative control. Create, edit, and link nodes." position="bottom">
          <button
            ...
            className={`p-2 rounded-lg transition-all duration-300 flex items-center justify-center ${mode === 'studio' ? 'bg-primary text-background shadow-[0_0_15px_var(--primary)]' : 'text-primary/40 hover:text-primary hover:bg-primary/5'}`}
          >
        ...
        <SmartTooltip content="Viewer Mode" description="Clean presentation mode. All editing tools are hidden." position="bottom">
          <button
            ...
            className={`p-2 rounded-lg transition-all duration-300 flex items-center justify-center ${mode === 'viewer' ? 'bg-primary text-background shadow-[0_0_15px_var(--primary)]' : 'text-primary/40 hover:text-primary hover:bg-primary/5'}`}
          >
        ...
        <div className="relative">
          <SmartTooltip content="Switch Theme" description="Choose a color palette for the entire flowchart studio." position="bottom">
            <button
              ...
              className={`p-2 rounded-lg transition-all duration-300 flex items-center justify-center ${showThemeMenu ? 'bg-primary/10 text-primary' : 'text-primary/40 hover:text-primary hover:bg-primary/5'}`}
            >
        ...
        <SmartTooltip content="Export Data" description="Download the current flowchart as a structured JSON file." position="bottom">
          <button
            ...
            className="p-2 rounded-lg text-primary/40 hover:text-primary hover:bg-primary/5 transition-all duration-300 flex items-center justify-center"
          >
      ...
      {mode === 'studio' && (
        <>
          ...
          {/* Toolbox */}
          <div className="bg-background/60 backdrop-blur-xl p-1 rounded-xl border border-primary/10 flex flex-col gap-1 shadow-2xl">
            <div className="px-1 py-0.5 border-b border-primary/5 mb-0.5">
              <span className="text-[8px] font-mono text-primary/20 uppercase tracking-widest">Toolbox</span>
            </div>
              ...
              {tools.map((tool) => (
                ...
                >
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-all duration-300 flex items-center justify-center ${activeTool === tool.id ? 'bg-primary text-background shadow-[0_0_15px_var(--primary)] scale-110' : 'text-primary/40 hover:text-primary hover:bg-primary/5'}`}
                  >
